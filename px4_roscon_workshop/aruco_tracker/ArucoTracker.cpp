...
// 	_image_sub = create_subscription<sensor_msgs::msg::Image>(
// 			     "/camera", qos, std::bind(&ArucoTrackerNode::image_callback, this,
// std::placeholders::_1));
...
// 				   std::bind(&ArucoTrackerNode::local_position_callback, this,
// std::placeholders::_1));
...
// void ArucoTrackerNode::local_position_callback(const
...
// 				// NOTE: we break here, meaning we only publish the pose of the
// first target we see 				break;
// 			}
...
// {
// 	// Always update the camera matrix and distortion coefficients from the new message
// 	_camera_matrix = cv::Mat(3, 3, CV_64F, const_cast<double*>(msg->k.data())).clone();   // Use
// clone to ensure a deep copy 	_dist_coeffs = cv::Mat(msg->d.size(), 1, CV_64F,
// const_cast<double*>(msg->d.data())).clone();   // Use clone to ensure a deep copy
...
// 	std::ostringstream stream;
// 	stream << std::fixed << std::setprecision(2);
// 	stream << "X: "  << target[0] << " Y: " << target[1]  << " Z: " << target[2] << " M: " <<
// _calculated_marker_size; 	std::string text_xyz = stream.str();
