...
using CustomModeWithExecutor =
    px4_ros2::NodeWithModeExecutor<CustomModeExecutor, CustomWaypoints, CustomYaw>;

static const std::string kNodeName = "CustomModeDemo";
static const bool kEnableDebugOutput = true;

CustomModeExecutor::CustomModeExecutor(px4_ros2::ModeBase& owned_mode,
...
void CustomModeExecutor::onDeactivate(DeactivateReason reason)
{
  const char* reason_str =
      (reason == DeactivateReason::FailsafeActivated) ? "failsafe activated" : "other reason";
...
void CustomModeExecutor::switchToState(State state, px4_ros2::Result previous_result)
{
  ...
  if (previous_result != px4_ros2::Result::Success) {
    RCLCPP_WARN(node().get_logger(), "Switching to state %d due to previous result: %d",
  ...
  switch (state) {
    case State::Takeoff:
      ...
      takeoff([this](px4_ros2::Result result) { switchToState(State::CustomWaypoints, result); },
              2.0f);
      break;
    ...
    case State::CustomYaw:
      ...
      break;
    case State::Land:
      land([this](px4_ros2::Result result) { switchToState(State::WaitUntilDisarmed, result); });
      break;
