...
class CustomWaypointsGoto : public px4_ros2::ModeBase {
 ...
 private:
  bool positionReached(const Eigen::Vector3f& target_position_m) const
  {
    ...
    const Eigen::Vector3f position_error_m = target_position_m - _local_position->positionNed();
...
int main(int argc, char* argv[])
{
  ...
  rclcpp::spin(
      std::make_shared<px4_ros2::NodeWithMode<CustomWaypointsGoto>>(kNodeName, kEnableDebugOutput));
  rclcpp::shutdown();
  return 0;
}
