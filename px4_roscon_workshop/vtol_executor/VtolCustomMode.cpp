...
VtolCustomMode::VtolCustomMode(rclcpp::Node& node)
    : px4_ros2::ModeBase(node, kModeName), _node(node)
{
  loadParameters();
  ...
  switch (_phase) {
    ...
    case Phase::Cruise:
      ...
      if ((_node.now() - _cruise_start_time).seconds() > _param_cruise_duration_s) {
        RCLCPP_INFO(_node.get_logger(),
                    "Cruise duration elapsed, commanding transition to multicopter");
    ...
    case Phase::TransitionToMulticopter:
      ...
      if (_vtol->getCurrentState() == px4_ros2::VTOL::State::Multicopter) {
        RCLCPP_INFO(_node.get_logger(),
                    "Transition to multicopter complete, completing VtolCruise mode");
